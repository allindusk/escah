...
- ⚠️ **角色名浮窗原位优先、句末为降级兜底**：zh 引擎对 `data-char` span（非头像 img、非 plugin-tooltip）默认**原位升级 `class=char-ref`**（文本填中文显示名），绝收进句末【】。`<span class="char-ref" data-char="日文名">【中文名】</span>` 浅描边胶囊、虚线下划线、`font-size:1em`。ja 站保留原位日文角色名（plugin-tooltip 蓝气泡），绝不显中文/移句末（用户原话"日文镜像页你也用句尾加超链接方案还把日文名翻译成中文名，你有病吧"）。⚠️ **但含角色名 span 的块若节点级 key 缺译、而整段 blk.zh 完整，必须改用「整段 blk.zh 作正文 + 句末补【角色名】浮窗」**（2026-08-15 修复）：否则节点级兜底显示日文 → 整段露日文（全站曾 3448 处同类 bug）。即：节点级全译→内联（原位）；节点级缺译+blk.zh 完整→句末浮窗兜底（不露日文）；blk.zh 也空→才忍痛日文。
